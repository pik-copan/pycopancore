...
* Variables that represent (changes of) stocks per time or time derivatives
  of stocks should be called **flows**, e.g. `photosynthesis_carbon_flow`
  of dimension `carbon/time` and default unit `GtC/yr`, or `income_flow`
  of dimension `money/time` and default unit `dollars/yr`.
...
* Other variables of dimension `1/time` should be called **rates**, e.g.
  ...
  `diffusion_rate`. Variables of other dimension than `1/time`, in
  particular fractions or ratios, should *not* be called 'rates'.
