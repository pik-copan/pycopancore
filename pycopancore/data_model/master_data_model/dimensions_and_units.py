...
class DimensionsAndUnits:
    ...
    mass = Dimension(
        "mass",
        "mass (only use for matter changing its type, otherwise use "
        "specific mass dimension, e.g. 'carbon')",
    )
    mass.default_unit = t = tonnes = Unit(
        "tonnes",
        "metric tonnes (only use for matter changing its type, "
        "otherwise use specific mass units, e.g. 'GtC')",
        symbol="t",
    )
    ...
    utility = Dimension(
        "utility",
        "abstract utility, representing an individual's preferences. "
        "BEWARE: this may not be considered to be interpersonally "
        "comparable!",
    )
    ...
    carbon = Dimension(
        "carbon", "mass of carbon, whether atomic or in some chemical compound"
    )
    ...
    J = joules = Unit(
        "joules", "(not convertible to kg m²/s²)", symbol="J", dimension=energy
    )
