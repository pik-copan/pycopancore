...
class Environment:
    ...
    land_area = Variable(
        "land area", "", unit=DAU.square_kilometers, strict_lower_bound=0
    )

    # Atmosphere:

    atmospheric_carbon = Variable(
    ...
    # Note: this is NOT the same as the CF var.
    # atmosphere_mass_of_carbon_dioxide

    surface_air_temperature = Variable(
    ...
    deep_ocean_carbon = Variable(
        "deep ocean carbon stock",
    ...
    biomass_carbon = Variable(
        "biomass/plant carbon stock",
        "(mass of C in any chemical compound)",
        unit=DAU.gigatonnes_carbon,
        is_extensive=True,
        lower_bound=0,
    ...
    # Note: when using the following, include
    # Implicit(biomass_carbon == harvestable_biomass_carbon +
    # other_biomass_carbon)
    harvestable_biomass_carbon = Variable(
        "harvestable biomass carbon stock",
    ...
    other_biomass_carbon = Variable(
        "other (non-harvestable) biomass carbon stock",
    ...
    # Note: when using the following, include
    # Implicit(fossil_carbon == harvestable_biomass_carbon +
    # other_biomass_carbon)
    discovered_fossil_reserves = Variable(
        "discovered fossil reserves carbon stock",
    ...
    undiscovered_fossil_reserves = Variable(
        "undiscovered fossil reserves carbon stock",
    ...
    ocean_atmosphere_diffusion_coefficient = Variable(
        "ocean-atmosphere diffusion coefficient",
        "",
        unit=DAU.years**-1,
        lower_bound=0,
        default=0.016,
    )
    carbon_solubility_in_sea_water = Variable(
        "carbon solubility in sea water",
    ...
    photosynthesis_carbon_flow = Variable(
        "photosynthesis carbon flow",
        "",
    ...
    terrestrial_respiration_carbon_flow = Variable(
        "plant and soil respiration carbon flow",
        "",
        unit=DAU.gigatonnes_carbon / DAU.years,
        is_extensive=True,
        lower_bound=0,
    ...
    plant_respiration_carbon_flow = Variable(
        "plant respiration carbon flow",
        "",
    ...
    soil_respiration_carbon_flow = Variable(
        "soil respiration carbon flow",
        "",
    ...
    geographic_network = Variable(
        "geographic network",
        """Undirected network of cells and geographic neighborhood.
                 Two cells should be linked iff they share a common boundary
                 of positive measure.""",
        datatype=Graph,
    )
