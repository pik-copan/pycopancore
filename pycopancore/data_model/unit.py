...
class Unit(object):
    ...
    def dimension(self):
        ...
        if self.is_base:
            ...
            return reduce(
                operator.mul,
                [unit.dimension**ex for unit, ex in self.exponents.items()],
                nondim,
            )
    ...
    def __init__(
        ...
        *,
        ...
        if is_base:
            ...
            # exponents
            self.name = (
                (str(self.factor) + " " if self.factor != 1 else "")
                + " ".join(
                    [
                        unit.name
                        + (
            ...
            if symbol is not None:
                ...
                items = sorted(
                    [(unit.symbol, ex) for unit, ex in exponents.items()],
            ...
            self.desc = (
                "\n\n".join(
                    [unit.name + ": " + unit.desc for unit in exponents.keys()]
                )
                if desc is None
                else desc
            )
    ...
    def convert(self, number, unit):
        assert unit.dimension == self.dimension, (
            "can't convert from " + str(self) + " to " + str(unit)
        )
    ...
    def __repr__(self):
        return (
            ("" if self.name in (None, "") else self.name + " ")
            + "["
            + self.symbol
            + "]"
        )
    ...
    def __pow__(self, power):
        ...
        return Unit(
            ...
            exponents={
                unit: ex * power for unit, ex in self.exponents.items()
            },
        )
        ...
        if isinstance(other, Unit):
            ...
            for unit, ex in oex.items():
                if unit in pex:
                    ...
                    pex[unit] = ex
            return Unit(
                is_base=False, factor=self.factor * other.factor, exponents=pex
            )
        else:
            return Unit(
                is_base=False, factor=self.factor * other, exponents=pex
            )
        ...
        if isinstance(other, Unit):
            ...
            for unit, ex in oex.items():
                if unit in qex:
                    ...
                    qex[unit] = -ex
            return Unit(
                is_base=False, factor=self.factor / other.factor, exponents=qex
            )
        else:
            return Unit(
                is_base=False, factor=self.factor / other, exponents=qex
            )

    def __rtruediv__(self, other):
        """non-unit / unit returns a DimensionalQuantity"""
        return dimensional_quantity.DimensionalQuantity(
            number=other, unit=self ** (-1)
        )

    def __rmul__(self, other):
        """non-unit * unit returns a DimensionalQuantity"""
        return dimensional_quantity.DimensionalQuantity(
            number=other, unit=self
        )
