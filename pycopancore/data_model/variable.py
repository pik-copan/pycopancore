...
class Variable(Symbol):
    ...
    (https://tntcat.iiasa.ac.at/ADVANCEWP1DB/static/download/
    Diagnostics_template_2015-08-12.xlsx)"""
    CETS = None
    ...
    def __new__(cls, name, *args, **assumptions):
        return super().__new__(cls, str(random.random()), **assumptions)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert not (
            is_extensive is True and is_intensive is True
    ...
    def __repr__(self):
        ...
        # FIXME: the following was commented out as a dirty fix of the
        # AttributeError in run_example1.py:
    ...
    def _check_valid(self, v):
        ...
        if self.array_shape is not None:
            if not v.shape == self.array_shape:
                return False, str(self) + " array shape must be " + str(
                    self.array_shape
                )
            for item in v:
        ...
        if v is None:
            ...
            # FIXME: the following was commented out since it does not
            # accept 0 as float:
            #            if self.datatype is not None:
            ...
            if self.lower_bound is not None:
                if not v >= self.lower_bound:
                    return False, str(self) + " must be >= " + str(
                        self.lower_bound
                    )
            ...
            if self.quantum is not None:
                if not v % self.quantum == 0:
                    return False, str(
                        self
                    ) + " must be integer number of " + str(self.quantum)

            if self.levels is not None:
                if v not in self.levels:
                    return False, str(self) + " must be in " + str(self.levels)
    ...
    def is_valid(self, value):
        ...
        return self._check_valid(value) == True  # noqa: E712
    ...
    def set_values(self, instances=None, values=None, *, dictionary=None):
    ...
    def clear_derivatives(self, *, instances=None):
    ...
    def get_derivatives(self, instances=None):
    ...
    def eval(self, instances=None, *, unit=None):
        ...
        """
        #        return [self.get_value(inst, unit=unit) for inst in
        #        instances]  # too slow...
