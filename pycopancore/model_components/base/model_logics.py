...
class ModelLogics(object):
    ...
    """dict giving for each explicit target variable the set of vars
    occurring on RHS of equation"""
    ODE_dependencies = None
    """dict giving for each ODE target variable the set of vars occurring
    on RHS of equation"""
    ...
    def configure(cls, reconfigure=False, **kwargs):
        ...
        cls.components = OrderedSet(
            [
                ...
                and not "name"  # noqa: E713
                in c.__dict__  # TODO: use a cleaner method than this to
                # distinguish interfaces from implementation?
            ]
        )
        ...
        for component in cls.components:
            ...
            for mixin in component.entity_types + component.process_taxa:
                ...
                for (
                    ...
                    mixin_interface.__dict__.items()
                ):  # k is the attribute's name (here the variable name),
                    # v the attribute's value (here the Variable object)
                    if isinstance(v, Variable):
                        if v not in variable_pool:  # we found a new one
                            ...
                            variable_pool.add(v)
                            print("        Variable ", v)
                        else:  # same Var. has been registered in another
                            # component or mixin already:
                            print("        Variable ", v)
            ...
            for k, v in variables:
                if isinstance(v, Variable):
                    assert v in variable_pool, (
                        "Variable "
                        + str(v)
                        + " was not defined in any component interface! "
                        "Maybe you forgot to include some entity type in "
                        "some model component's Model.entity_types?"
                    )
                    # this version is shorter and easier to understand than
                    # the cryptic .format(**locals()) version, so why the
                    # hell change it???
                    #                    assert v.codename == k, \
                    #                        "Variable '{v!r}' was registered "
        ...
        for composed_class in cls.entity_types + cls.process_taxa:
            ...
            for c in parents:
                if (
                    "processes" in c.__dict__ and c.processes is not None
