...
class Model (object):
    ...
    description = "For each of a list of traits (=combinations of variables), a certain fraction of entities of the variable's owning type looks at a certain number of fraction of their neighbors on a certain network and imitates their variable values with certain probabilities when certain thresholds are met. " \
    "All these specifics are governed by parameters in Culture that can be overridden in imitating entity types by providing respective variables or methods there. " \
    "E.g., to override the imitation rate for the trait with key 'tax', the entity type can either define a Variable or method named imi_rate_tax." \
    "To use this component, the model composer must first import model_components.config, then set config.generic_imitation['variables'] to a list of variables that this component shall act on, and only then import model_components.generic_imitation. " \
    "Each entry in config.generic_imitation['variables'] must be a dot construct starting at base.interface.Culture and leading to the variable in question. " \
    "E.g., if the variables Individual.is_environmentally_friendly and SocialSystem.emissions_tax_level shall be imitated, put config.generic_imitation['variables'] = [base.interface.Culture.individuals.is_environmentally_friendly, base.interface.Culture.social_systems.emissions_tax_level]." 
...
class Culture (object):
    ...
    imi_traits = Variable(
        "imitation traits",
        "Dict of key: trait. Each trait is a tuple of one or more Variables that shall be imitated together. Each occurring Variable must also be listed in the config variable generic_imitation['variables']. See this model component's Model.description for more information on this. All other imi_* parameters defined in Culture are allowed to be dictionaries keyed with the keys listed here, or with the key '*' for specifying values to be applied to all (other) entries occurring here. For some of them, their values can also be overridden by imitating entity types by providing variables or methods named imi_p_imitate_<key>, imi_p_imitate_<key> etc.",
    ...
    imi_type = Variable(
        ...
        "'threshold' means entities draw several neighbors and imitate each observed value occurring above a certain threshold with some probability.",
    ...
    imi_p_in_batch = Variable(
        ...
        "(Dict of) probability/ies with which entities are selected to be in an imitation batch. Alternatively, imi_batch_n can be specified, but not both. " \
        "Can be overridden by imitating entity types by providing variables or methods named imi_p_in_batch_<key> where <key> is a trait key defined in imi_traits.",
        datatype = Union[float, Dict[str, float]],
    ...
    imi_p_imitate = Variable(
        ...
        "Can be overridden by imitating entity types by providing a variables or methods named imi_p_imitate_<key> where <key> is a trait key defined in imi_traits. " \
