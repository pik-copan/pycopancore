...
class _DotConstruct(sp.AtomicExpr):
    ...
    at the start of the dot construct, e.g. SocialSystem or
    Environment.cells"""
    ...
    def __getattr__(self, name):
        ...
        if self._argument is not None:  # we are an aggregation with argument
            ...
            #            print("adding argument",argument,"to aggregation of
            #            type",self._aggregation)
            newdc = _DotConstruct(
                self._start,
                self._attribute_sequence,
                aggregation=self._aggregation,
    ...
    def __call__(self, *args, **kwargs):
        """calling is only allowed if we are an aggregation without
        argument yet, and results in adding an argument"""
    ...
    def target_variable(self):
        ...
        #            print("getting target variable of",self,"as",
...
def _eval(expr, iteration=None):
    ...
    # binary operators:
        ...
        assert np.all(condvals[1]), (
            "Piecewise only works with args of the form (iftrue, cond), "
            "(iffalse, True) yet!"
        )
        truthvals = condvals[0]
        trues = list(
            np.where(truthvals == True)[0]  # noqa: E712
        )  # "==" is correct here, since it may be a sympy.True!! Do not
        # replace "==" by "is"!!
        vals = piecevals[1]
    ...
    # Following: = True if even no. of arguments is True = Not(Xor):
        ...
        #        base[np.where(np.logical_and(np.abs(base) < EPS,
        #        exponent < 0))] = EPS
        #        # try to avoid overflows due to (large abs)**(positive):
        #        base[np.where(np.logical_and(np.abs(base) > LARGE,
        #        exponent > 0))] = LARGE
        #        # try to avoid invalid values due to
        #        # (negative)**(non-integer):
        #        base[np.where(np.logical_and(base < 0,
        #        exponent % 1 != 0))] = EPS
