...
class Hooks(object):
    ...
    """
    _mid_hooks = {}
    """dict of hooks that are executed every time the run is halted for a
    step or an event
    ...
    def register_hook(cls, type, hook, theclass=None):
        ...
        if type is cls.Types.pre:
            ...
            # if the Code ends up here, there is an error in the
            # implementation because cls.HookTypes has been extended but there
            # is no registration done here
    ...
    def unregister_hook(
        ...
        try:
            ...
            if error_if_not_registered:
                raise HookRegistrationError(
                    "hook is not listed as registered, so it cannot be "
                    "unregistered"
                )
            # else: ignore quietly
