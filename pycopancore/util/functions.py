...
def safe_pow(x, e, i=np.inf):
    ...
    return (
        sp.Piecewise(
            (i, x == 0 > e), (sp.sign(x) * sp.Pow(sp.Abs(x), e), True)
        )
        if isinstance(x, sp.Expr)
        else i if x == 0 > e else np.sign(x) * np.abs(x) ** e
    )
...
def safe_div(a, b, i=np.inf, z=0):
    ...
    """
    return (
        sp.Piecewise((sp.sign(a) * i, b == 0 != a), (z, b == 0), (a / b, True))
        if isinstance(a, sp.Expr) or isinstance(b, sp.Expr)
        else np.sign(a) * i if b == 0 != a else z if b == 0 else a / b
    )
...
def safe_log(x, i=-np.inf):
    ...
    """
    return (
        sp.Piecewise((sp.log(x), x > 0), (i, True))
        if isinstance(x, sp.Expr)
        else np.log(x) if x > 0 else i
    )
