...
societies = [M.Society(world=world,
...
# distribute global aggregate quantities to cells and societies:
...
ax1.plot(t[3:], 100*average(array([traj[M.Society.has_renewable_subsidy][s][3:] for s in societies]), axis=0), 
         color="orange", lw=2, label="societies with renewable subsidy")
ax1.plot(t[3:], 100*average(array([traj[M.Society.has_emissions_tax][s][3:] for s in societies]), axis=0), 
         color="blue", lw=2, label="societies with emissions tax")
ax1.plot(t[3:], 100*average(array([traj[M.Society.has_fossil_ban][s][3:] for s in societies]), axis=0), 
         color="black", lw=2, label="societies with fossil ban")
ax1.set_ylabel('percent')
ax1.legend()

for i, s in enumerate(societies):
    ax2.semilogy(t[3:], traj[M.Society.population][s][3:], color="yellow", lw=2, label="population [humans]" if i==0 else None)
    ax2.semilogy(t[3:], traj[M.Society.physical_capital][s][3:], color="black", lw=2, label="physical capital [$]" if i==0 else None)
    ax2.semilogy(t[3:], traj[M.Society.renewable_energy_knowledge][s][3:], color="orange", lw=2, label="renewable energy knowledge [GJ]" if i==0 else None)
